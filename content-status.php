...
if ( is_tax() or is_front_page() ) {
    ...
    <article <?php post_class(); ?> <?php if ( is_single() ) {
        ...
        $enough_avatar = sprintf( $enough_content_date_html, $enough_content_permalink, get_avatar( get_the_author_meta( 'user_email' ), 48 )
        );

        $enough_content_html = sprintf( $enough_content_html, sprintf( $enough_content_date_html, $enough_content_permalink, $enough_status_date . $enough_status_time
                ), apply_filters( 'enough_post_thumbnail', get_the_post_thumbnail( null, 'post-thumbnail' )
                ), $enough_content, enough_post_format_posted_in( false )
        );

        if ( is_front_page() ) {
            $enough_post_format = get_post_format();

            printf( '<p><a class="post-format-link" href="%1$s"><span>%2$s</span></a></p>', esc_url( get_post_format_link( $enough_post_format ) ), enough_get_post_format_string( $enough_post_format )
            );
        }


        printf( $enough_html, $enough_avatar, $enough_status_title, $enough_content_html
        );
    ...
    <article <?php post_class(); ?> <?php if ( is_single() ) {
        ...
        if ( is_single() ) {
            ...
            printf( '<a class="post-format-link" href="%1$s"><span>%2$s</span></a>', esc_url( get_post_format_link( $enough_post_format ) ), enough_get_post_format_string( $enough_post_format )
            );
        }
